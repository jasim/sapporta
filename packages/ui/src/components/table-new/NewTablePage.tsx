...
import { NewTableGrid } from "./NewTableGrid";
import { NewTableToolbar } from "./NewTableToolbar";
import { Pagination } from "../table/Pagination";
import { useSchemaStore } from "../../stores/schema-store";
import { openDrawerCreate } from "../../stores/dispatchers";
import { getApiBase } from "../../client";
import { buildRowsQuery } from "../../services/rows";
import {
  parseTableSearchParams,
  buildTableSearchParams,
} from "../../routes/url-helpers";
import { getNavigate } from "../../stores/router-bridge";
import { loadPref, savePref } from "../../lib/local-prefs";
import type { ColId, SortDescriptor } from "@/grid";
import { createNewTable, type NewTableHandle } from "./new-table-state";
import { startTableLookupLoading } from "./table-lookup-loading";
import { registerNewTable, unregisterNewTable } from "./new-grid-registry";
...
// Top-level entry for the new-grid path. Owns:
//   - the URL-synced state store + REST-backed grid runtime (`NewTableHandle`)
...
//
export function NewTablePage({ tableName }: { tableName: string }) {
  ...
  const [handle, setHandle] = useState<NewTableHandle | null>(null);
  ...
  useEffect(() => {
    ...
    registerNewTable(tableName, h);
    const stopLookupLoading = startTableLookupLoading(h);
    setHandle(h);

    return () => {
      stopLookupLoading();
      unregisterNewTable(tableName);
      h.dispose();
      setHandle(null);
    };
  ...
  return <NewTablePageInner handle={handle} tableSchema={tableSchema} />;
}
...
function useSourceField<T>(
  handle: NewTableHandle,
  pick: (snap: ReturnType<NewTableHandle["source"]["snapshot"]>) => T,
): T {
