...
import {
  ...
  type TableFilter,
  type TableLevelMeta,
} from "@/table/grid-adapter/compile-table-grid";
import { createTableGridThemeContext } from "@/table/grid-adapter/table-grid-theme-context";
import {
...
export type TableState = {
  ...
  page: number;
  pageSize: number;
  // Sticky banner text surfaced in `TablePage` above the grid. Driven
...
export function createTable(args: CreateTableArgs): TableHandle {
  const pkCol = args.tableSchema.columns.find((c) => c.primary);
  if (!pkCol) {
    throw new Error(
      `createTable: table '${args.tableName}' has no primary key column - every Sapporta table must declare one`,
    );
  }
  ...
  const store = createStore<TableState>()((set, get) => ({
