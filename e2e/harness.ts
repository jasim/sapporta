...
export function writeTasksSchema(projectDir: string): void {
  ...
  writeFileSync(
    ...
    [
      'import { sapportaTable, timestamp, sqliteTable, text, integer } from "@sapporta/server/table";',
      "",
...
export function writeAuthMatrixSchema(projectDir: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      '  id: integer("id").primaryKey({ autoIncrement: true }),',
      '  name: text("name").notNull(),',
      '  tier: text("tier", { enum: ["standard", "priority"] }).notNull(),',
      '  workspace_id: text("workspace_id").notNull(),',
      "});",
      "",
      ...
      '  status: text("status", { enum: ["draft", "sent", "paid"] }).notNull(),',
...
export function writeProjectsSchema(projectDir: string): void {
  ...
  writeFileSync(
    ...
    [
      ...
      '  id: integer("id").primaryKey({ autoIncrement: true }),',
      '  name: text("name").notNull(),',
      '  status: text("status", { enum: ["active", "paused", "done"] }).notNull(),',
      '  workspace_id: text("workspace_id").notNull(),',
      '  created_at: timestamp("created_at"),',
      '  updated_at: timestamp("updated_at"),',
      "});",
